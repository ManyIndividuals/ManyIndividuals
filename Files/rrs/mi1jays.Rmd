...
Feeders were set up in the field, counterbalanced for location, and were available for dispensing food when the experiment was being conducted (Figures 3,4). Individuals were habituated to the open feeders filled with food until at least half of the minimum sample size visited at least 1 feeder. Individuals that participated in the experiment in a social group started on the same rewarded option for the initial discrimination to improve motivation for participating in the experiment (e.g., if the first rewarded option was counterbalanced across individuals, subjects might be slower to learn their rewarded option if they use social information about which option is rewarded). If the subject visited the rewarded feeder, via a remote controlled by the experimenter, the feeder delivered a small amount of food and then closed to reset more food in preparation for the next opening. If the subject visits the non-rewarded feeder, the presence data was recorded, but the feeder did not open. All feeders contained one type of high value food. In addition, feeders were opaque and always had food in them to eliminate the confound due to olfactory differences between the feeders that could be introduced if only the rewarded feeders had food in them. If a feeder needed to be refilled, the experimenter refilled all feeders consecutively in the same time period and for the same amount of time even if that feeder did not need much or any food (in these cases, we pretended to fill the feeder). This eliminates confounds from cues provided by a differential amount of attention experimenters give the feeders depending on which needed refilling.

Data are collected on success measures (see below) before and after the flexibility intervention.

**Passing criteria:**

 - **Feeder habituation:** all feeders at all locations contained visible food and were open for several hours daily or until at least half of the minimum sample size visited at least one of the feeders.
 - **Reversal passing criterion:**  an individual was considered to have a preference when they chose 10 of the most recent 12 trials (choices) correct (the rewarded option). This criterion applied to the initial discrimination, and to each reversal.
 - **Manipulation passing criterion:** passed two consecutive reversals in 12 trials or less (see Serial reversal passing criterion in the Supplementary materials, S2).


### Assessment of the likelihood of success in human modified environments with regard to the flexibility manipulation

Here, we compared pre- and post-flexibility manipulation measures on the same individuals using the following success measures. We chose population-relevant success variables that are predicted to be the most likely to be used in human modified environments. Observational methods to collect these data are described below (i.e., all occurrences sampling).
...
Food types are broken down into plant (seed, fruit, human-provided, or unknown plant) and animal (insect larva, adult insect, crustacean, amphibian, reptile, mammal, bird, egg, human-provided, or unknown animal). “Human-provided” indicates any food item that was acquired from a store at some point and is left out by humans. For example, sunflower seeds would be considered human-provided if they are in the form of bird seed or a human snack. Sunflower seeds would only be counted in the “seed” category if the bird is seen eating it from a plant. Data were collected on the four plant subcategories and ten animal subcategories and used in the analyses.
...
Additionally, for all observational methods we used binoculars so that we could stay far enough from the focal individual that our presence is not affecting their behavior. Because that distance can be different for each individual and species, we hesitate to give a specific number. However, if at any point the focal individual showed that it was affected by our presence by looking directly at the researcher, alarm calling or startling, then we ended the observation immediately, dropped all data from that observation and attempted the observation again the next day.

We attempted to balance observations in the morning and afternoon for all individuals in the study. In this way, we collected a random sample of data from active and inactive time periods for all individuals. 

#### All occurrences sampling

We collected data on the foraging and microhabitat use of jays during spatial movement tracking (the latter as part of another investigation). We followed jays for 60 minutes and recorded at 1-minute intervals the spatial location (GPS coordinates), any food items consumed, and the microhabitat the jay is present in. We conducted 4 tracks per individual before, and 4 tracks per individual after the flexibility manipulation.
...
Many individuals from our focal population of Florida scrub-jays were already color-banded. We additionally caught 24 jays over 2 field seasons using walk in traps and mist nets. We collected biometric data, a blood sample, applied colored leg bands and attached a radio tag using a leg loop harness to a small subset of individuals, then released them at their point of capture. We tested out the radio tag technology on four individuals in the first field season. However, the solar panel on the tags had low functionality, reducing the usability of this technology in this system. The Florida scrub-jays are highly territorial, and we were always able to find the jays to collect observational data without the radio tags.

We conducted the serial reversal manipulation on 9 individuals. We will collect data on pre- and post-manipulation success measures and conduct the experiment within the non-breeding season to control for potential temporal differences in the environment and behavior.
