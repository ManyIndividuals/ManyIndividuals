...
```{r plotjayflexbeforehabitatcomparison, eval=F}
...
library(lme4)
sl = glmer(adj.max ~ Location+Reversal + (1|ID) + (1|olre), family = "poisson", data = dflex2)
summary(sl) # no difference with max.trial or adj.max
boxplot(max.trial ~ Location+Reversal, data = dflex2)
s0 = glmer(adj.max ~ Location + (1|olre), data = dflex2[which(dflex2$Reversal == 0),], family = "poisson")
summary(s0) # no difference with max.trial or adj.max
overdisp_fun(s0)
...
```{r plotjayhabitatuse, eval=F}
pp.div = read.csv(url("https://raw.githubusercontent.com/ManyIndividuals/ManyIndividuals/refs/heads/main/Files/rrs/mi1_jays_pp_div.csv"), header=T, sep=",", stringsAsFactors=F)

boxplot(forage ~ pp, data = pp.div)
boxplot(height ~ pp, data = pp.div)
boxplot(substrate ~ pp, data = pp.div)

pp.div2 = melt(pp.div, id.vars = c("ID","Condition","pp"),value.name = "value", measure.vars = c("forage","substrate","height"))
pp.div2$pp = factor(pp.div2$pp, levels = c("pre","post"))
colnames(move2) = c("ID","pp","value")
move2$variable = "step.length"
pp.div3 = pp.div2
pp.div3 = pp.div3[-which(pp.div3$variable == "height"),]

pp.div3 = rbind(pp.div3,move2)

ggplot(pp.div3, aes(pp,value, group = ID)) +
  facet_wrap(~ variable, scales = "free") +
  geom_line() +
  geom_point(size = 3, aes(color = pp)) +
  theme_classic() +
