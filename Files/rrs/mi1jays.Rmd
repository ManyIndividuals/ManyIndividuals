...
**The model**
We used poisson regression to assess whether jays in human-modified areas foraged on more different kinds of foods and used more substrate types relative to jays in natural areas. We included a random effect for jay ID and an offset for the amount of time each jay was observed.
...
## J.Q1 Do jay populations in human modified areas differ in baseline behavioral flexibility compared to populations in natural areas? 
There was no difference in baseline behavioral flexibility performance between jays in suburban or natural areas. This was true when we considered either of the two behavioral flexibility components, $\phi_j$, the rate of learning the newly rewarded option ($\beta$ = -0.23, sd = 1.27, CI = -2.22 - 1.4), or $\lambda_j$, the rate of deviation from previously rewarded options ($\beta$ = -0.01, sd = 0.18, CI = -0.28 - 0.28).
...
## J.Q4 More flexible = more food types?
Similarly, after the serial reversal flexibility training, jays were not more likely to eat a greater diversity of food types ( = 0.26, CI = -0.45 - 0.96, p > 0.05; Fig. 4).
...
# **ANALYSIS PLAN - Power analyses simulations and model code**
...
```{r stage1-jayflexsppdiff, eval=F}
library(rethinking)
library(cmdstanr)
set_ulam_cmdstan(TRUE) #makes it so ulam always runs with cmdstan
...
```{r stage1-jayflexbeforehabitatcomparison, eval=F}
library(rethinking)
library(cmdstanr)
set_ulam_cmdstan(TRUE) #makes it so ulam always runs with cmdstan
...
```{r jayflexpopdiff, eval=F}
## This is an unregistered analysis to test whether FLSJ persisting in human-modified areas (suburban neighborhoods) show more flexible foraging and habitat use behaviors compared to FLSJ in natural areas where human disturbance is absent. ##
...
```{r stage1-plotjayhabitatuse, echo=FALSE}
pp.div = read.csv(url("https://raw.githubusercontent.com/ManyIndividuals/ManyIndividuals/refs/heads/main/Files/rrs/mi1_jays_pp_div2.csv"), header=T, sep=",", stringsAsFactors=F)
colnames(pp.div)[3] = "food"
library(reshape2)
pp.div2 = melt(pp.div, id.vars = c("ID","Condition","pp"),value.name = "value", measure.vars = c("food","substrate","height"))
