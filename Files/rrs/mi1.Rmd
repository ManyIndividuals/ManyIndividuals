...
dat <- list(
    habitatuse = rbinom(10000,size=10,prob=5/10), #10 max habitats; on average, individuals will use 5 of the 10 habitats more than 5% of the time
    tid = c(rep(1,N_ind),rep(2,N_ind)), #1 refers to before and 2 to after
    actor = rep(1:N_ind,2) #go through all individuals twice because there are observations from before and after for each
)
...
d[d$Habitat=="Natural",]$habitats<-+3
...
precis(mj,depth=2) #pay attention to g to determine whether there is a difference between the before and after conditions (there is a difference if CI is on one side of zero)
