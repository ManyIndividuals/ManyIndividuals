...
Different criteria exist to decide whether an individual has learned an association during a reversal. The two main two criteria are to switch an individual after it either has chosen 10 out of 12 choices correct or 17 out of 20 choices. The criteria are further modified depending on whether choices are assessed continuously or grouped in predetermined blocks. 

Here, we assess whether stopping after an individual chose 10 out of the last 12 continuously counted choices can be used as a reliable reversal passing criterion. To determine reliability and suitability, we investigated five questions by generalizing previously simulated reversal learning data from @xpop, which was based on reversal learning data from one species (great-tailed grackles). Grackles are fast to reverse preferences compared with many other species [@Logan2016], therefore we generalized the simulations to other species by setting the parameters that guide performance (phi and lambda) to also lead to slower performances. 

The findings from these simulated data indicate that deciding that an individual has passed the reversal when they choose 10 out of the last 12 consecutive trials correctly is functional and reliable, because with this criterion:

*1) individuals will be finished after fewer trials than with other criteria*
With the 10 out of 12 criterion, individuals pass the reversal 8 trials earlier (median) than with the 17 out of 20 criterion. This means that for most individuals the two rules are equally effective as they will pass them in the same run of trials, but because the 10 out of 12 criterion is restricted to 12 trials instead of 20 individuals need 8 fewer trials. No individual needs more trials with the 10 out of 12 criterion. When trials are grouped into blocks (of 10), individuals need 5 more trials (median) compared to when choices are assessed continously.

*2) classification of individuals is less noisy because there is less of a chance of individuals coming close to the criterion but not passing, including the risk that some individuals never pass*
The average improvement is larger than the median because, first, there are no individuals who are faster with the 17 out of 20 criterion, and secondly because there is a subset of individuals who need considerably fewer trials with the 10 out of 12 criterion (Figure m1Fig_ReversalCriterionComparison.png). Individuals with very larger number of trials (>100) to pass almost never occur with the 10 out 12 criterion, whereas they are more common with the 17 out of 20. With more trials, there is a higher chance that an individual will deviate from their preference by chance. This is also reflected by the 65 out of the 626 individuals who never reached the 17 out of 20 criterion within the maximum 300 trials that were simulated, whereas there were only 4 individuals with the 10 out of 12 criterion. Accordingly, an additional benefit of choosing the 10 out of 12 criterion is that it is more likely that data for all individuals, even those slow to learn an association, can be collected.

m1Fig_ReversalCriterionComparison.png

*3) variation among individuals is still present and similar to the variation detected with other criteria*
As described above, when changing the criterion from 17 out of 20 to 10 out of 12, most individuals need 8 fewer trials. That also means that the differences among individuals, which might contain relevant information about variation among them, is preserved. When transforming performance with the two criteria to ranks, individuals are sorted essentially in the same order independent of which of the criteria is being used.

*4) individuals can be assumed to have reliably learned the association*
Based on the two criteria, we can extract the attractions that simulated individuals have formed toward both the rewarded and the unrewarded option at the point when they would be assumed to have passed. Comparing the two attractions, we can determine whether individuals are likely to have learned an association or not. Independent of the criterion, individuals generally have formed a preference for the rewarded option (89% of individuals favour the rewarded over the unrewarded option between 2.5 and 14 times). The lowest relative preference are very similar between the two options, suggesting that these are individuals who bias their choice even when there are only small differences in attraction, which again indicates that they have reliably learned the association. With the 10 out of 12 criterion, individuals would in the next trial on average choose the rewarded option with a probability of 76% (3 times more likely to choose rewarded over unrewarded option), whereas this is 84% with the 17 out of 20 criterion (5 times more likely).
...
As expected based on the relative attraction scores at the end of the reversal (probability to choose the now rewarded option of 76% for 10/12 and 84% for 17/20), most individuals choose the now rewarded option in 4 or less out of the first 12 trials after a passing the previous reversal, lower than if they had formed no association and slightly higher than how individuals would perform that would have passed with the 17 out of 20 criterion (Fig m1Fig_ReversalCriterionAssociation.png). The probability that an individual would, after a reversal, directly reach the 10 out of 12 rule by chance is 0.001, and even these individuals will have reversed their association because they will have updated their attractions during the 12 trials.



m1Fig_ReversalCriterionComparison.png

m1Fig_ReversalCriterionAssociation.png
...
################################################################################################
# ii) With the 10 correct out of 12 continuous trials criterion, variation among individuals is still present and similar to the variation detected with other criteria
################################################################################################
...
################################################################################################
# iii) With the 10 correct out of 12 continuous trials criterion, classification is less noisy because there is less of a chance of individuals missing the criterion repeatedly or even never reaching it, meaning that individuals slow to learn will be more reliably classified than with a different criterion
################################################################################################
...
################################################################################################
# iv) With the 10 correct out of 12 continuous trials criterion, individuals can be assumed to have reliably learned the association
################################################################################################
...
################################################################################################
# iv) this learned association means that individuals who move to the next reversal with this criteria are unlikely to solve the reversed association by chance.
################################################################################################
...
We estimated our power to detect differences between conditions at different sample sizes and with different mean changes in the total number of different microhabitats used per individual in the before vs. after conditions (Figure 4). We simulated the number of habitats used for different sample sizes of individuals before and after the flexibility manipulation. We analyzed these simulated data with the model we will use to analyze the actual data, estimating the change in the number of habitats used between the before and after conditions. From the posterior estimates of the model, we extracted both the mean change as well as the ratio of the posterior estimates that were below zero.

If the mean ratio of estimates below zero is close to 0.5, the model assumes that the change in the number of habitats used before the flexibility manipulation is similar to after. If the ratio is close to zero, the model assumes individuals have changed their behavior. For changes in the number of habitats used smaller than 2, models are likely to assume that no changes occurred even with large sample sizes. If the change in the number of habitats used before the flexibility manipulation vs. after is 2 with a standard deviation of 2, on average 91% of the posterior of the model based on a sample size of 20 individuals will be larger than zero. This means that the model is quite certain there is a difference that is larger than zero. In addition, only 6 of the 30 models for a sample size of 20 at the mean change of 2 have a ratio larger than 0.3, meaning that the risk of having a false negative is not very likely.

In general, with sample sizes at or above 20 and mean changes in the number of habitats used at 2 or larger, it is likely that the model will indicate that individuals have changed their behavior. Mean changes below 2 can still be detected, however there is a much higher risk that there will be a false negative and this risk is independent of sample size. For example, 60% of the models for a sample size of 20 at the mean change of 1 have a ratio larger than 0.3, meaning there is a risk of having a false negative.
...
plot(NULL,xlim=c(-0.2,5),ylim=c(0.0,0.8),pch=1,ann=F,frame.plot=F,cex=1.5)

mu_10<- link( m_samplesize_diffscount2 , data=data.frame( changenumber_mean=sqrt(unique(d2$changenumber_mean)),samplesize=rep(1,length(unique(d2$changenumber_mean))) ) )
mu_10_mean <- apply( mu_10 , 2 , mean )
mu_10_ci <- apply( mu_10 , 2 , PI , prob=0.97 )
...
mu_20 <- link( m_samplesize_diffscount2 , data=data.frame( changenumber_mean=sqrt(unique(d2$changenumber_mean)),samplesize=rep(2,length(unique(d2$changenumber_mean))) ) )
mu_20_mean <- apply( mu_20 , 2 , mean )
mu_20_ci <- apply( mu_20 , 2 , PI , prob=0.97 )
...
mu_30 <- link( m_samplesize_diffscount2 , data=data.frame( changenumber_mean=sqrt(unique(d2$changenumber_mean)),samplesize=rep(3,length(unique(d2$changenumber_mean))) ) )
mu_30_mean <- apply( mu_30 , 2 , mean )
mu_30_ci <- apply( mu_30 , 2 , PI , prob=0.97 )
...
mu_40 <- link( m_samplesize_diffscount2 , data=data.frame( changenumber_mean=sqrt(unique(d2$changenumber_mean)),samplesize=rep(4,length(unique(d2$changenumber_mean))) ) )
mu_40_mean <- apply( mu_40 , 2 , mean )
mu_40_ci <- apply( mu_40 , 2 , PI , prob=0.97 )
...
mu_60 <- link( m_samplesize_diffscount2 , data=data.frame( changenumber_mean=sqrt(unique(d2$changenumber_mean)),samplesize=rep(5,length(unique(d2$changenumber_mean))) ) )
mu_60_mean <- apply( mu_60 , 2 , mean )
mu_60_ci <- apply( mu_60 , 2 , PI , prob=0.97 )
...
# Axis labels (side 1=x axis, side 2=y axis)
mtext("A. Power to detect differences between conditions: SD=2",side=3,line=1,at=3,cex=1.3)
mtext("Mean change in number of habitats used before vs after",side=1,line=3, cex=1)
mtext("Ratio of estimates below zero",side=2,line=3, cex=1.2)

#92% of the posterior mass is above zero for a 2 or larger mean change in number of habitats used between before and after when the sample size is 20
#mu_10_ci #91%. column 4 for the 2 category on the x axis (ordered by unique(d$changenumber_mean))
#mu_20_ci #92%
#mu_30_ci #93%
#mu_40_ci #93%
#mu_60_ci #94%
#sum(d2[d2$n %in% 20 & d2$changenumber_mean %in% 2,]$proportion_estimates_below_zero<0.3) #24 values are <0.3 (out of 30)
...
mtext("B. SD=1",side=3,line=1,at=0.5,cex=1.3)
mtext("Mean change in number of habitats used before vs after",side=1,line=3, cex=1)
par(op)

#94% of the posterior mass is above zero for a 2 or larger mean change in number of habitats used between before and after when the sample size is 20
#mu1_10_ci #92%. column 4 for the 2 category on the x axis (ordered by unique(d$changenumber_mean))
#mu1_20_ci #93%
#mu1_30_ci #94%
#mu1_40_ci #94%
#mu1_60_ci #94%
#sum(d1[d1$n %in% 20 & d1$changenumber_mean %in% 2,]$proportion_estimates_below_zero<0.3) #28 values are <0.3 (out of 30)
...
We estimated our power to detect differences between conditions at different sample sizes and with different mean changes in the total number of different food types taken per individual in the before vs. after conditions (Figure 4). We simulated the number of food types taken for different sample sizes of individuals before and after the flexibility manipulation. We analyzed these simulated data with the model we will use to analyze the actual data, estimating the change in the number of food types taken between the before and after conditions. From the posterior estimates of the model, we extracted both the mean change as well as the ratio of the posterior estimates that were below zero.

If the mean ratio of estimates below zero is close to 0.5, the model assumes that the change in the number of food types taken before the flexibility manipulation is similar to after. If the ratio is close to zero, the model assumes individuals have changed their behavior. For changes in the number of food types taken smaller than 2, models are likely to assume that no changes occurred even with large sample sizes. If the change in the number of food types taken before the flexibility manipulation vs. after is 2 with a standard deviation of 2, on average 91% of the posterior of the model based on a sample size of 20 individuals will be larger than zero. This means that the model is quite certain there is a difference that is larger than zero. In addition, only 6 of the 30 models for a sample size of 20 at the mean change of 2 have a ratio larger than 0.3, meaning that the risk of having a false negative is not very likely.

In general, with sample sizes at or above 20 and mean changes in the number of food types taken at 2 or larger, it is likely that the model will indicate that individuals have changed their behavior. Mean changes below 2 can still be detected, however there is a much higher risk that there will be a false negative and this risk is independent of sample size. For example, 60% of the models for a sample size of 20 at the mean change of 1 have a ratio larger than 0.3, meaning there is a risk of having a false negative.
...
changenumber <- rnorm(N_ind, mean=(13-numberbefore)-4.5,sd=2) #if had a large number before, have a smaller change relative to the after condition. Subtract 4.5 to have a change of 2 food types (13-6.5 = 6.5-4.5 = 2)
...
# Set the means you want to compare. To get a certain mean change in number of microhabitats between before and after, set the subtractor in the changenumber mean (as above): a small subtractor means a large change
meanchangenumber1<-1.5 #individuals will on average take 5 more foods after the experiment (13-6.5 = 6.5 - 1.5  = 5)
meanchangenumber2<-2.5 # individuals will on average take 4 more foods after the experiment (13-6.5 = 6.5 - 2.5  = 4)
meanchangenumber3<-3.5 #individuals will on average take 3 more foods after the experiment (13-6.5 = 6.5 - 3.5  = 3)
meanchangenumber4<-4.5 # individuals will on average take 2 more foods after the experiment (13-6.5 = 6.5 - 4.5  = 2)
meanchangenumber5<-5.5 # individuals will on average take 1 more food after the experiment (13-6.5 = 6.5 - 5.5  = 1)
meanchangenumber6<-6.5 # individuals will on average take 0 more foods after the experiment (13-6.5 = 6.5 - 6.5  = 0)
changenumbervariants<-c(meanchangenumber1,meanchangenumber2,meanchangenumber3,meanchangenumber4,meanchangenumber5,meanchangenumber6) #for false negatives
#changenumbervariants<-c(0) #for false positives
#changenumbervariants<-c(meanchangeprop6) #for showing differences
...
for (samplesize in 1:length(samplesizes)) {
        ...
        #sets up the data sheet with the simulated data that the model will run on
      dat <- list(
    foods = standardize(c(numberbefore,numberafter)), #compare before and after
    before = c(rep(1,N_ind),rep(2,N_ind)), #1 refers to before and 2 to after
    ind = rep(1:N_ind,2) #go through all individuals twice because there are observations from before and after for each
)
...
post <- extract.samples(sjf)
  ...
  # enter all of the results into the data sheet
    simresults[counter,]$n <- N_ind
    simresults[counter,]$changenumber_mean <- 5.5 - currentchangenumber
    simresults[counter,]$changenumber_sd <- 1 #change this to match the SD in changenumber
    simresults[counter,]$repetition <- repetition
    simresults[counter,]$number_before <- currentnumber_before
    simresults[counter,]$proportion_estimates_below_zero <- sum(post$beta<0)/2000
    simresults[counter,]$count_estimates_below_zero <- sum(post$beta<0)
...
#write.csv(simresults,file="simresultsjayforagingsd2.csv") #put the outputs into a csv file so don't have to run the model again...it takes hours
#write.csv(simresults,file="simresultsjaysforagingd1.csv")
```

```{r jayq4simulationfiguresamplesize, eval=T, messages=F}
library(rethinking)
library(cmdstanr)
set_ulam_cmdstan(TRUE) #makes it so ulam always runs with cmdstan
...
**Figure 5.** Curves of the mean ratio of estimates below zero (vertical lines show the minimum and maximum ratios), which illustrates the power we have to detect differences between conditions at different sample sizes. Across all models, the standard deviation of the mean change in number of food types taken was 2 (A) or 1 (B), and the number of food types taken before the flexibility manipulation was 6.5.
