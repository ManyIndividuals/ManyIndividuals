...
#Make a vector with all the possible pairwise distances between the mean change in proportion between before and after
changeprop1 <- rnorm(N_ind, mean=meanchangeprop1,sd=0.1)
changeprop2 <- rnorm(N_ind, mean=meanchangeprop2,sd=0.1)
changeprop3 <- rnorm(N_ind, mean=meanchangeprop3,sd=0.1)
changeprop4 <- rnorm(N_ind, mean=meanchangeprop4,sd=0.1)
changeprop5 <- rnorm(N_ind, mean=meanchangeprop5,sd=0.1)
changeprop6 <- rnorm(N_ind, mean=meanchangeprop6,sd=0.1)

pairwisecomparisons<-c(meanchangeprop1,meanchangeprop2,meanchangeprop3,meanchangeprop4,meanchangeprop5,meanchangeprop6)
# Calculate the differences between populations. We set this up in a vector that will link to the simulation output, so there are 50 repeats for the 10 independent repetitions across the 5 different sample sizes
currentoutcome<-NA
counter<-1

# We now run our simulations. There are 5 different sample sizes, and for each we perform 10 estimations for a total of 50 samples.
for (samplesize in 1:50) {
  
        n <- samplesizes[counter] #number of grackles in total across all sites
        sites <- matrix(nrow=n*5*2,ncol=2) #store data here
        colnames(sites)<-c("phi","sites") # We label the outcome phi, but this doesn't matter for the lambda simulations
        
        # We randomly draw the pre-specified number of individuals from the 10,000 individuals in each population
        sample1_1<-sample(changeprop1,n)
        sample2_1<-sample(changeprop2,n)
        sample3_1<-sample(changeprop3,n)
        sample4_1<-sample(changeprop4,n)
        sample5_1<-sample(changeprop5,n)
        sample6_1<-sample(changeprop6,n)

        # We combine them into one long dataframe
        allsamples<-c(sample1_1,sample2_1,sample3_1,sample4_1,sample5_1)
        
        # In the dataframe, we will have the phi, and an identifier for each population
        sites[,1]<-allsamples
        sites[,2]<-c(rep(1,n),rep(2,n),rep(3,n),rep(4,n),rep(5,n),rep(6,n),rep(7,n),rep(8,n),rep(9,n),rep(10,n))
    ...
    actor = rep(1:N_ind,2) #go through all individuals twice because there are observations from before and after for each
        ...
        # THE MODEL (p.441 for normal distribution, 448 for binomial distribution)
s1 <- ulam(
    alist(
...
#precis(m1,depth=2)
        ...
        post1$diff <- post1$g[,sample] - post1$g[,comparison]
...
mu_20 <- link( m_samplesize_populationdifferences , data=data.frame( populationdifference=sequence_mean_differences,samplesize=rep(4,length(sequence_mean_differences)) ) )
mu_20_mean <- apply( mu_20 , 2 , mean )
mu_20_ci <- apply( mu_20 , 2 , PI , prob=0.97 )
shade( 1-mu_20_ci , exp(sequence_mean_differences) , col=col.alpha("#213940",0.4) )

mu_60 <- link( m_samplesize_populationdifferences , data=data.frame( populationdifference=sequence_mean_differences,samplesize=rep(6,length(sequence_mean_differences)) ) )
mu_60_mean <- apply( mu_60 , 2 , mean )
mu_60_ci <- apply( mu_60 , 2 , PI , prob=0.97 )
shade( 1-mu_60_ci , exp(sequence_mean_differences) , col=col.alpha("yellow",0.4) )
