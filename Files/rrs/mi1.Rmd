...
#### Research question: Do flexibility manipulated individuals differ in the proportion of time spent at cafes and garbage dumpsters when food is present?
...
**Table 1.** Study design for the great-tailed grackle research. References that were not already cited in the introduction: [@barton2000limits].
...
number_before<-5 #5 habitats used per bird in the before condition
changenumber<-1.5 #there is a difference of 2 habitats used between the before and after conditions
...
#before
numberbefore <- round(rnorm(N_ind,mean=5,sd=1.5),2) #number of habitats used, 2 means 2 decimal points
numberbefore[numberbefore<1]<-1 #this needs to be a minimum of 1 bc an individual can never live in less than 1 habitat
changenumber <- rnorm(N_ind, mean=numberbefore/2,sd=1.5) #if had a large number before, have a smaller change relative to the after condition. Divide by 2 = assume that on average they will improve by is 2 habitats (the before mean of 5 divided by 2)
...
#check the plot to see whether individuals are increasing their numbers = yes, all points are above the 1:1 line
...
number_before<-c(2,3,4) #c(0.5,0.6,0.7)
...
sj <- ulam(
    alist(
        ...
        alpha ~ normal(5,2), #5=mean number of habitats used, 2=habitats for the sd
...
dat <- list(
    habitatuse = rnorm(40,mean=5,sd=2), #estimate 20 total individuals (*2 conditions) with a mean of 5 habitats/individual
    before = c(rep(1,N_ind),rep(2,N_ind)), #1 refers to before and 2 to after
    ind = rep(1:N_ind,2) #go through all individuals twice because there are observations from before and after for each
)
