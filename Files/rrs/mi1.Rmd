...
#choosing the mean proportion change in the amount of time spent at a location when a reward was present and standard deviations - visualize to see whether it would be possible to tell the difference between before (black) and after (red)
prob_before<-0.6
changeprop<-0.025
plot(density(rnorm(10000,prob_before,sd=0.1)),lwd=2)
...
samplesizes<-c(9,12,15,20,60)

# Set the means you want to compare
meanchangeprop1<-0.2 #e.g., a bird spends 0.6 of its time being rewarded at cafes and dumpsters before the manipulation and then increases this proportion to 0.8 after the manipulation, showing that the manipulation improved their foraging success
meanchangeprop2<-0.1
meanchangeprop3<-0.05
meanchangeprop4<-0.03
meanchangeprop5<-0.025
meanchangeprop6<-0.01
changepropvariants<-c(meanchangeprop1,meanchangeprop2,meanchangeprop3,meanchangeprop4,meanchangeprop5,meanchangeprop6)
...
prop_before<-c(0.5,0.6,0.7)


#set up data frame where results will be stored
simresults <- matrix(NA,nrow=900,ncol=6)
simresults <- data.frame(simresults)
colnames(simresults) <- c("n","changeprop_mean","changeprop_sd","repetition","proportion_before","proportion_estimates_below_zero")
counter<-1
...
#write.csv(simresults,file="simresults60.csv") #put the outputs into a csv file so don't have to run the model again...it takes hours
```
...
#head(d)
#plot(d$proportion_estimates_below_zero~d$changeprop_mean)

# We run the STAN model - it is a binomial model that assumes that the probability to estimate a difference depends on an interaction between the sample size and the difference between the conditions (before/after)
...
m_samplesize_diffs <- ulam(
    alist(
        proportion_estimates_below_zero ~ dbinom(1,p),
        logit(p) <- a[samplesize] + b[samplesize]*changeprop_mean,
        a[samplesize] ~ dnorm(a_bar,sigma_a),
        a_bar~dnorm(0,1.5),
        sigma_a~dexp(1),
        b[samplesize]~dnorm(0,1.5)
    ) , data=simresults , chains=4 , cores=4 , log_lik=TRUE , cmdstan = TRUE)
...
precis_output_5<-precis( m_samplesize_diffs , depth=2 )

# We can now plot the predictions for the different sample sizes as separate lines
plot(NULL,xlim=c(0,max(changeprop_mean),ylim=c(0.0,1.0),pch=1,ann=F,frame.plot=F,cex=1.5)

mu_9 <- link( m_samplesize_diffs , data=data.frame( populationdifference=changeprop_mean,samplesize=rep(1,length(changeprop_mean)) ) )
mu_9_mean <- apply( mu_9 , 2 , mean )
mu_9_ci <- apply( mu_9 , 2 , PI , prob=0.97 )
shade( 1-mu_9_ci , exp(changeprop_mean) , col=col.alpha("blue",0.4) )

mu_12 <- link( m_samplesize_diffs , data=data.frame( populationdifference=changeprop_mean,samplesize=rep(2,length(changeprop_mean)) ) )
mu_12_mean <- apply( mu_12 , 2 , mean )
mu_12_ci <- apply( mu_12 , 2 , PI , prob=0.97 )
shade( 1-mu_12_ci , exp(changeprop_mean) , col=col.alpha("orange",0.4) )

mu_15 <- link( m_samplesize_diffs , data=data.frame( populationdifference=changeprop_mean,samplesize=rep(3,length(changeprop_mean)) ) )
mu_15_mean <- apply( mu_15 , 2 , mean )
mu_15_ci <- apply( mu_15 , 2 , PI , prob=0.97 )
shade( 1-mu_15_ci , exp(changeprop_mean) , col=col.alpha("skyblue",0.4) )

mu_20 <- link( m_samplesize_diffs , data=data.frame( populationdifference=changeprop_mean,samplesize=rep(4,length(changeprop_mean)) ) )
mu_20_mean <- apply( mu_20 , 2 , mean )
mu_20_ci <- apply( mu_20 , 2 , PI , prob=0.97 )
shade( 1-mu_20_ci , exp(changeprop_mean) , col=col.alpha("#213940",0.4) )

mu_60 <- link( m_samplesize_diffs , data=data.frame( populationdifference=changeprop_mean,samplesize=rep(6,length(changeprop_mean)) ) )
mu_60_mean <- apply( mu_60 , 2 , mean )
mu_60_ci <- apply( mu_60 , 2 , PI , prob=0.97 )
shade( 1-mu_60_ci , exp(changeprop_mean) , col=col.alpha("yellow",0.4) )
...
mtext("Power to detect differences between conditions",side=3,line=1,at=0.025,cex=1.5)
mtext("The mean change in proportion of time spent in the presence of a reward between before and after conditions",side=1,line=3)
mtext("Proportion of estimates below zero",side=2,line=3)
```

**Figure X.** A difference in the proportion of time an individual spends at a location when food is present before the flexibility manipulation vs. after can be detected if the sample size is at least 20.
```{r gtgrq1model, eval=T}
library(rethinking)
library(cmdstanr)
set_ulam_cmdstan(TRUE) #makes it so ulam always runs with cmdstan
