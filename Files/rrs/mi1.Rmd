...
prob_control<-c(0.5,0.6,0.7)

#set up data frame where results will be stored
simresults <- matrix(NA,nrow=900,ncol=9)
simresults <- data.frame(simresults)
colnames(simresults) <- c("n","changeprob_mean","changeprob_sd","repetition","survival_control","proportion_estimates_below_zero_condition","count_estimates_below_zero_condition","proportion_estimates_below_zero_habitatnatural","proportion_estimates_below_zero_habitatsuburban")
counter<-1
...
for (samplesize in 1:length(samplesizes)) {
        ...
        #create data frame to store for each repetition of the simulated data to be analyzed by the model
        sites <- matrix(nrow=n,ncol=3) 
        colnames(sites)<-c("ID","condition","survival")
        
        #CONTROL & MANIP groups
        probcontrol <- round(rbinom(n=N_ind/2,size=1,prob=currentprob_control),2) #for half the data points, randomly assign a 0 (dead) or 1 (alive) survival status according to the baseline survival probabilities (0.5,0.6,0.7)
        probcontrol[probcontrol>1]<-1 #values larger than 1 mess up the model so restrict it
        
        probmanip <- round(rbinom(n=N_ind/2,size=1,prob=currentprob_control+currentchangeprob),2) #for the other half the data points, randomly assign a 0 (dead) or 1 (alive) survival status according to the baseline survival probabilities (0.5,0.6,0.7)
        probmanip[probmanip>1]<-1 #values larger than 1 mess up the model so restrict it
  ...
  dat2 <- list(
    ...
    habitat = rbinom(N_ind,1,0.25)+1, #75% of individuals in the reserve, 25% in suburban randomly assigned from control and manip groups. 1=reserve, 2=suburban
...
#write.csv(simresults,file="simresultsTouSD0.1.csv") #put the outputs into a csv file so don't have to run the model again...it takes about 5 hours

## Prep a figure to show how sample size and mean change prop makes a difference in the results. Get posterior values for particular sample sizes and change props. Set samplesize (20 or 60), changeprobvariants (0.1 or 0.025), and prop_control (always 0.6) to just one number and run the model. Generate the output, write to csv. Repeat for other model variants
#write.csv(diff_p,file="diff_p20changeprob0.1.csv")
#write.csv(diff_p,file="diff_p60changeprob0.1.csv")
#write.csv(diff_p,file="diff_p20changeprob0.025.csv")
#write.csv(diff_p,file="diff_p60changeprob0.025.csv")
```
