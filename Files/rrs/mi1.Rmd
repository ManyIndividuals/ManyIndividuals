...
dat_full <- as.list(dflex_beginning)
dat_full$N <- nrow(dflex_beginning) #total number of rows
dat_full$N_id <- length(unique(as.factor(dflex_beginning$ID)))
dat_full$N_exp <- length(unique(dflex_beginning$Habitat))
...
dat_full$Habitat <- as.numeric(as.factor(dflex_beginning$Habitat)) 
...
m_simulated_full <- stan( model_code =  reinforcement_model_id_site_nonzeroattraction, data=dat_full ,iter = 10, cores = 4, chains=4, control = list(adapt_delta=0.9, max_treedepth = 12)) 
