...
(duration that food was present / total duration at cafes and dumpsters) ~ Time (before/after) + (1 | ID)

```{r gtgr, eval=T}
# Use Arizona reversal learning data to make priors (n=20 grackles in first and last reversals)
revfirst <- c(50,70,100,80,50,60,40,160,90,70,80,80,60,100,40,60,80,60,70,100) 
revlast <- c(50,70,100,80,50,60,40,160,50,50,40,40,23,50,30,40,80,80,70,100)
mean(revfirst) #75 trials
mean(revlast) #63 trials
mean(revfirst) - mean(revlast) #12 trials
sd(revfirst) #27 trials
sd(revlast) #31 trials
sd(revfirst) - sd(revlast) #-4 trials
cor.test(revfirst,revlast) #0.79

a <- 74 #avg first reversal trials to pass
b <- (-12) #avg difference between first and last reversal trials to pass (avg last=63 including both control and manipulated data)
sigma_a <- 27 #std dev in intercepts (std dev first reversal trials to pass)
sigma_b <- 4 #std dev in slopes (std dev first - std dev last)
rho <- 0.79 #correlation between intercepts and slopes (corr between first and last reversals)
...
N_condition <- 2 #before/after

library(MASS)
vary_effects <- mvrnorm( N_condition , Mu , Sigma )

a_condition <- vary_effects[,1]
b_condition <- vary_effects[,2]

#figure that shows the intercepts and slopes as well as the distribution's contours
plot( a_condition , b_condition , col=4 , xlab="intercepts (a_condition)" , ylab="slopes (b_condition)" )
