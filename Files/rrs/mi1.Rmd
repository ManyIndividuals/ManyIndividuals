...
meanchangenumber1<-1 #individuals will on average use 4.5 more habitats after the experiment ((10-numberbefore)*(4.5/4.5) = push everyone to 10 meaning that the lowest individuals will increase by 9 and the highest by 0 food items. Therefore, the average is 4.5 food items
meanchangenumber2<-0.89 # 4 more habitats ((10-numberbefore)*(4/4.5)  = 4)
meanchangenumber3<-0.67 #3 more habitats ((10-numberbefore)*(3/4.5)  = 3)
meanchangenumber4<-0.44 # 2 more habitats ((10-numberbefore)*(2/4.5)  = 2)
meanchangenumber5<-0.22 # 1 more habitat ((10-numberbefore)*(1/4.5) = 1)
meanchangenumber6<-0 # 0 more habitats ((10-numberbefore)*(0/4.5)  = 0). 0 for false positives
...
post <- extract.samples(sj)
  ...
  # enter all of the results into the data sheet
    simresults[counter,]$n <- N_ind
    simresults[counter,]$changenumber_mean <- (10-currentnumber_before)*(currentchangenumber)
...
#write.csv(simresults,file="simresultsjayhabitatsd2.csv") #put the outputs into a csv file so don't have to run the model again...it takes hours
...
dat2 <- list(
  proportion_estimates_below_zero = as.numeric(d$proportion_estimates_below_zero),
  count_estimates_below_zero = as.numeric(d$count_estimates_below_zero),
  changenumber_mean = as.factor(d$changenumber_mean),
  samplesize = as.integer(as.factor(d$n))
)
...
#97% of the posterior mass is above zero for a 2 or larger mean change in number of habitats used between before and after when the sample size is 20
#mu_10_ci #93%. column 4 for the 2 category on the x axis (ordered by unique(d$changenumber_mean))
#mu_20_ci #97%
#mu_30_ci #98%
#mu_40_ci #99%
#mu_60_ci #99%
#sum(d2[d2$n %in% 20 & d2$changenumber_mean %in% 2,]$proportion_estimates_below_zero<0.3) #30 values are <0.3 (out of 30)
#length(d2[d2$n %in% 20 & d2$changenumber_mean %in% 2,]$proportion_estimates_below_zero) #30 values total
...
#98% of the posterior mass is above zero for a 2 or larger mean change in number of habitats used between before and after when the sample size is 20
#mu1_10_ci #96%. column 4 for the 2 category on the x axis (ordered by unique(d$changenumber_mean))
#mu1_20_ci #98%
#mu1_30_ci #94%
#mu1_40_ci #99.991%
#mu1_60_ci #99.997%
