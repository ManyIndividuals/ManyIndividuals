...
**Figure 2.** Comparing the four species involved in this investigation relative to their geographic range and association with human modified habitats. The yellow dots represent field site locations.
...
#### Research question (G.Q1): Do flexibility manipulated individuals differ in the proportion of time spent at cafes and garbage dumpsters when food is present?

We will investigate this question by tracking their presence at cafes and dumpsters when food is present versus when it is absent before and after manipulating their flexibility using serial reversal learning in the wild (Figure 3). Table 1 summarizes our predictions, analysis plans, interpretations for the various directions the results could go, and the theories that could be contradicted given the various outcomes.
...
#### Research question (J.Q1): Do jay populations in human modified areas differ in baseline behavioral flexibility compared to populations in natural areas?

We will investigate this question by comparing performance on serial reversal learning in the wild between jays in natural areas and jays in human modified areas. Table 2 summarizes our predictions, analysis plans, interpretations for the various directions the results could go, and the theories that could be contradicted given the various outcomes.

#### Research question (J.Q2): Are disturbance-resilient (DR) jays more behaviorally flexible than disturbance-sensitive (DS) jays?

We will investigate this question by comparing performance on serial reversal learning in the wild between between DR and DS jay species. Table 2 summarizes our predictions, analysis plans, interpretations for the various directions the results could go, and the theories that could be contradicted given the various outcomes.

#### Research question (J.Q3): Does manipulating behavioral flexibility alter the number of microhabitats used?

We will investigate this question by tracking their presence in a variety of microhabitats before and after manipulating their flexibility using serial reversal learning in the wild. Table 2 summarizes our predictions, analysis plans, interpretations for the various directions the results could go, and the theories that could be contradicted given the various outcomes.

#### Research question (J.Q4): Does manipulating flexibility alter the number of different food items taken by jays?

We will investigate this question by tracking the various food items they take before and after manipulating their flexibility using serial reversal learning in the wild. Table 2 summarizes our predictions, analysis plans, interpretations for the various directions the results could go, and the theories that could be contradicted given the various outc

<img width="100%" src="mi1Figgtgr.png">

**Figure 4.** The reversal learning experiment in a group context (Figure 7) tailored to the jay research questions.
...
CASJ/BLJA will use the same set up as the grackles (see above). **FLSJ...**

**Feeder habituation:** All feeders will have food available and be open for 3 hours per day on 5 consecutive days or until at least 20 banded individuals have visited at least 1 feeder.
...
##### J.Q2 Are CASJ more flexible than FLSJ?
...
Number of different habitats per individual ~ Condition + (Condition | ID)

Time is before or after the flexibility manipulation. ID is nested within time as a random effect because there is more than one data point per individual: each individual has a data point at time 1 and time 2.

*Bayesian model for a normal distribution:*

y ~ $\alpha$[ind] + $\beta$[ind]*before

y is the response variable: the total number of different habitats used per individual. There will be one intercept, $\alpha$, and one slope $\beta$ per individual, which will be estimated for the two conditions, before (and after) the manipulation. A normal distribution was used because the response variable is a sum without an expected skew to the curve [see Figure 10.6 in @mcelreath2020statistical]. The Bayesian model was developed using @mcelreath2020statistical as a guide.

###### Power analysis

We estimated our power to detect differences between conditions at different sample sizes and with different mean changes in the total number of different microhabitats used per individual in the before vs. after conditions (Figure 3). We simulated the number of habitats used for different sample sizes of individuals before and after the flexibility manipulation. We analyzed these simulated data with the model we will use to analyze the actual data, estimating the change in the number of habitats used between the before and after conditions. From the posterior estimates of the model, we extracted both the mean change as well as the ratio of the posterior estimates that were below zero.

If the mean ratio of estimates below zero is close to 0.5, the model assumes that the change in the number of habitats used before the flexibility manipulation is similar to after. If the ratio is close to zero, the model assumes individuals have changed their behavior. For changes smaller than 1, models are likely to assume that no changes occurred even with large sample sizes. If the change in the number of habitats used before the flexibility manipulation vs. after is 1, on average 83% of the posterior of the model based on a sample size of 20 individuals will be larger than zero. This means that the model is fairly certain there is a difference that is larger than zero. In addition, none of the models for a sample size of 20 at the mean change of 1 have a ratio larger than 0.3, meaning that the risk of having a false negative is unlikely.

In general, with sample sizes at or above 20 and mean changes in the number of habitats used at 1 or larger, it is likely that the model will indicate that individuals have changed their behavior. Mean changes below 1 can still be detected, however there is a higher risk that there will be a false negative. If the change in the number of habitats used before the flexibility manipulation vs. after is 0.025, on average 61% of the posterior of the model based on a sample size of 20 individuals will be larger than zero. In addition, only 20% of the models for a sample size of 20 at the mean change of 0.025 have a ratio larger than 0.3, meaning that the risk of having a false negative is still low.

With small mean changes in the response variable, some individuals might not increase or even decrease their response after the manipulation because there is variation around the mean change in individual responses. With small sample sizes, there is a risk that only individuals who did not clearly increase their response will be studied, whereas larger sample sizes are more likely to include a wider spectrum of individuals.

To estimate the risk of detecting false positives, we set the mean change in the number of habitats used to zero so there was no change between the before and after conditions. As expected, the average ratio of estimates below zero is close to 0.5 and independent of sample size. With a sample size of 20, 43% have a ratio smaller than 0.3, meaning that the risk of having a false positive is high. The risk would be lower if the variation among individuals was lower than what we assumed (across all models, the standard deviation of the mean change in number of habitats was 1, which is a conservative estimate).
...
lines(density(rnorm(10000,number_before*(changenumber*1),sd=1.5)),lwd=2,col="red") #the after condition. Should be able to tell the difference when changenumber is at least 1.5 and the SD is at least 1.5 (and number_before is 5)
...
changenumber <- rnorm(N_ind, mean=(10-numberbefore)-3.5,sd=1.5) #if had a large number before, have a smaller change relative to the after condition. Subtract 3.5 to have a change of 2 habitats (10-4.5=5.5-3.5=2)
...
# Set the means you want to compare. To get a certain mean change in number of microhabitats between before and after, set the subtractor in the changenumber mean (as above): a small subtractor means a large change
meanchangenumber1<-0.5 #individuals will on average use 5 more habitats after the experiment (10-4.5 = 5.5 - 0.5  = 5)
meanchangenumber2<-1.5 # individuals will on average use 4 more habitats after the experiment (10-4.5 = 5.5 - 1.5  = 4)
meanchangenumber3<-2.5 #individuals will on average use 3 more habitats after the experiment (10-4.5 = 5.5 - 2.5  = 3)
meanchangenumber4<-3.5 # individuals will on average use 2 more habitats after the experiment (10-4.5 = 5.5 - 3.5  = 2)
meanchangenumber5<-4.5 # individuals will on average use 1 more habitats after the experiment (10-4.5 = 5.5 - 4.5  = 1)
meanchangenumber6<-5.5 # individuals will on average use 0 more habitats after the experiment (10-4.5 = 5.5 - 5.5  = 0)
...
#changenumbervariants<-c(0) #for false positives
#changenumbervariants<-c(meanchangeprop6) #for showing differences
...
for (samplesize in 1:length(samplesizes)) {
        ...
        changenumber <- rnorm(N_ind, mean=(10-numberbefore)-currentchangenumber,sd=1) #change this SD to explore the boundaries of individual variation in their response (in terms of the success measure) to the flexibility manipulation. We explored SD=1 and SD=2
...
#str(post) #check it
#diff_b <- post$b_ind[,2] - post$b_ind[,1] #log-odds difference p.341 McElreath 2020. Don't need to do the contrast because we already have each individual's number so we only need to look at their beta to see whether it is positive (more habitats after) or negative (fewere habitats after)
#PI(diff_b) #did the difference cross zero?
#precis( list( diff_b=diff_b ) )

  # enter all of the results into the data sheet
    simresults[counter,]$n <- N_ind
    simresults[counter,]$changenumber_mean <- 5.5 - currentchangenumber
    simresults[counter,]$changenumber_sd <- 1
    simresults[counter,]$repetition <- repetition
...
#write.csv(simresults,file="simresultsjaysd2.csv") #put the outputs into a csv file so don't have to run the model again...it takes hours
#write.csv(simresults,file="simresultsjaysd1.csv")

## Prep a figure to show how sample size and mean change number makes a difference in the results. Get posterior values for particular sample sizes and change numbers. Set samplesize (20 or 60), changenumbervariants (0.1 or 0.025), and number_before (always 0.6) to just one number and run the model. Generate the output, write to csv. Repeat for other model variants
#write.csv(diff_b,file="diff_p20changenumber2.csv")
```

```{r jayq3simulationfiguresamplesize, eval=T, messages=F}
library(rethinking)
library(cmdstanr)
set_ulam_cmdstan(TRUE) #makes it so ulam always runs with cmdstan
...
d <- d[d$changenumber_sd==2,]

# Set up the data sheet for the model
dat <- list(
  proportion_estimates_below_zero = as.numeric(d$proportion_estimates_below_zero),
  count_estimates_below_zero = as.numeric(d$count_estimates_below_zero),
  changenumber_mean = as.numeric(sqrt(d$changenumber_mean)),
  samplesize = as.integer(as.factor(d$n))
)
...
dat1 <- list(
  ...
  changenumber_mean = as.numeric(sqrt(d1$changenumber_mean)),
...
plot(NULL,xlim=c(-0.2,5.2),ylim=c(0.0,0.8),pch=1,ann=F,frame.plot=F,cex=1.5)

mu_10<- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d$changenumber_mean)),samplesize=rep(1,length(unique(d$changenumber_mean))) ) )
mu_10_mean <- apply( mu_10 , 2 , mean )
mu_10_ci <- apply( mu_10 , 2 , PI , prob=0.97 )
shade( mu_10_ci , unique(d$changenumber_mean) , col=col.alpha("blue",0.7) )
lines(x=c(-0.1,-0.1),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(0.9,0.9),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(1.9,1.9),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(2.9,2.9),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(3.9,3.9),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(4.9,4.9),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))

mu_20 <- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d$changenumber_mean)),samplesize=rep(2,length(unique(d$changenumber_mean))) ) )
mu_20_mean <- apply( mu_20 , 2 , mean )
mu_20_ci <- apply( mu_20 , 2 , PI , prob=0.97 )
shade( mu_20_ci , unique(d$changenumber_mean) , col=col.alpha("orange",0.7) )
lines(x=c(-0.05,-0.05),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(0.95,0.95),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(1.95,1.95),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(2.95,2.95),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(3.95,3.95),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(4.95,4.95),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))

mu_30 <- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d$changenumber_mean)),samplesize=rep(3,length(unique(d$changenumber_mean))) ) )
mu_30_mean <- apply( mu_30 , 2 , mean )
mu_30_ci <- apply( mu_30 , 2 , PI , prob=0.97 )
shade( mu_30_ci ,unique(d$changenumber_mean), col=col.alpha("skyblue",0.7) )
lines(x=c(0,0),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(1,1),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(2,2),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(3,3),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(4,4),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(5,5),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))

mu_40 <- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d$changenumber_mean)),samplesize=rep(4,length(unique(d$changenumber_mean))) ) )
mu_40_mean <- apply( mu_40 , 2 , mean )
mu_40_ci <- apply( mu_40 , 2 , PI , prob=0.97 )
shade( mu_40_ci ,unique(d$changenumber_mean) , col=col.alpha("#213940",0.7) )
lines(x=c(0.05,0.05),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(1.05,1.05),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(2.05,2.05),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(3.05,3.05),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(4.05,4.05),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(5.05,5.05),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))

mu_60 <- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d$changenumber_mean)),samplesize=rep(5,length(unique(d$changenumber_mean))) ) )
mu_60_mean <- apply( mu_60 , 2 , mean )
mu_60_ci <- apply( mu_60 , 2 , PI , prob=0.97 )
shade( mu_60_ci , unique(d$changenumber_mean), col=col.alpha("red",0.7) )
lines(x=c(0.1,0.1),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(1.1,1.1),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 1,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(2.1,2.1),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 2,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(3.1,3.1),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 3,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(4.1,4.1),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 4,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(5.1,5.1),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 5,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))

legend(x="topright",legend=c(lty2="10",lty1="20",lty3="30",lty2="40",lty3="60"),lty=c(1,1,1,1,1),lwd=c(5,5,5,5,5),col=c(col.alpha("blue",0.4),col.alpha("orange",0.4),col.alpha("skyblue",0.4),col.alpha("#213940",0.4),col.alpha("red",0.4)),cex=1,bty="n")

# Axis labels (side 1=x axis, side 2=y axis)
mtext("Sample size",side=3,line=-0.7,at=5.1)
mtext("Power to detect differences between conditions (SD=2)",side=3,line=1,at=2,cex=1.5)
mtext("Mean change in number of habitats used before vs. after",side=1,line=3, cex=1.2)
mtext("Ratio of estimates below zero",side=2,line=3, cex=1.2)

#83% of the posterior mass is above zero for a 1 or larger mean change in number of habitats used between before and after when the sample size is 20
#mu_10_ci #83%. column 5 for the 1 category on the x axis (ordered by unique(d$changenumber_mean))
#mu_20_ci #83%
#mu_30_ci #85%
#mu_40_ci #85%
#mu_60_ci #86%
#sum(d[d$n %in% 20 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero<0.3) #13 values are <0.3 (out of 30)
#length(d[d$n %in% 20 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero) #30 values total


# plot for sd=1
plot(NULL,xlim=c(-0.2,5.2),ylim=c(0.0,0.8),pch=1,ann=F,frame.plot=F,cex=1.5)

mu1_10<- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d1$changenumber_mean)),samplesize=rep(1,length(unique(d1$changenumber_mean))) ) )
...
mu1_20 <- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d1$changenumber_mean)),samplesize=rep(2,length(unique(d1$changenumber_mean))) ) )
...
mu1_30 <- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d1$changenumber_mean)),samplesize=rep(3,length(unique(d1$changenumber_mean))) ) )
...
mu1_40 <- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d1$changenumber_mean)),samplesize=rep(4,length(unique(d1$changenumber_mean))) ) )
...
mu1_60 <- link( m_samplesize_diffscount , data=data.frame( changenumber_mean=sqrt(unique(d1$changenumber_mean)),samplesize=rep(5,length(unique(d1$changenumber_mean))) ) )
...
# Axis labels (side 1=x axis, side 2=y axis)
mtext("Sample size",side=3,line=-0.7,at=5.1)
mtext("Power to detect differences between conditions (SD=1)",side=3,line=1,at=2,cex=1.5)
mtext("Mean change in number of habitats used before vs. after",side=1,line=3, cex=1.2)
mtext("Ratio of estimates below zero",side=2,line=3, cex=1.2)
par(op)
...
#mu_40_ci #99%
#mu_60_ci #99.7%
#sum(d[d1$n %in% 20 & d1$changenumber_mean %in% 0,]$proportion_estimates_below_zero<0.3) #13 values are <0.3 (out of 30)
#length(d1[d1$n %in% 20 & d1$changenumber_mean %in% 0,]$proportion_estimates_below_zero) #30 values total
```

**Figure 4.** Curves of the mean ratio of estimates below zero (vertical lines show the minimum and maximum ratios), which illustrates the power we have to detect differences between conditions at different sample sizes. Across all models, the standard deviation of the mean change in proportion was 0.1 (a conservative estimate) and the proportion of time spent in the presence of food before the flexibility manipulation was 0.6.
...
#model output: pay attention to beta to determine whether there is a difference between the before and after conditions (there is a difference if CI on one side of zero)
precis(mj,depth=2)
...
Number of different food types eaten per individual ~ Habitat * Time + (Time | ID)

Habitat is either a human modified or a natural site, and time is before or after the flexibility manipulation. ID is nested within time as a random effect because there is more than one data point per individual: each individual has a data point at time 1 and time 2.
...
dat <- list(
    foods = rnorm(40,mean=10,sd=2), #estimate 40 total food types with a mean of 10 types eaten/individual
    B = c(rep(1,N_ind),rep(2,N_ind)), #1 refers to before and 2 to after
    ind <- rep(1:N_ind,2) #go through all individuals twice because there are observations from before and after for each
)

## when the data exist, load data sheet
d <- read.csv(url(""), header=T, sep=",", stringsAsFactors=F)

# Set up the data sheet
dat <- list(
    foods = d$foods,
    B = d$BeforeOrAfterManipulation,
    ind = d$ID
)

# THE MODEL (p.441 for normal distribution - McElreath 2020)
mj <- ulam(
    alist(
        foods ~ normal( mu , sigma ),
        mu <- alpha[ind] + habitat[] + condition[ind]*B,
        c(alpha,b)[ind] ~ multi_normal( c(alpha,habitat) , Rho , sigma_ind ),
        alpha ~ normal(5,2),
        habitat ~ normal(-1,0.5),
        condition ~ normal(-1,0.5),
        sigma_ind ~ exponential(1),
        sigma ~ exponential(1),
        Rho ~ lkj_corr(3)
    ) , data=dat , chains=4 , cores=4 , log_lik=TRUE , cmdstan = TRUE)
...
dat <- list(
    ...
    fledglingsex = d$Sex,
    caregiversex = d$Sex,
    hatchdate = as.numeric(d$HatchDate),
