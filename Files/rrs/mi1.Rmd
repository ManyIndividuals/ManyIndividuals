...
samplesizes<-c(20,30,40,60) #c(10,20,30,40,60)
...
#write.csv(simresults,file="simresults20.csv") #put the outputs into a csv file so don't have to run the model again...it takes hours
```

```{r gtgrq1simulationfiguresamplesize, eval=F}
library(rethinking)
library(cmdstanr)
set_ulam_cmdstan(TRUE) #makes it so ulam always runs with cmdstan
...
#mean(d$proportion_estimates_below_zero)
...
dat <- list(
  ...
  changeprop_mean = d$changeprop_mean,
...
m_samplesize_diffscount <- ulam(
  alist(
    count_estimates_below_zero~dbinom(10000,p),
...
precis_output<-precis( m_samplesize_diffs , depth=2 )

# We can now plot the predictions for the different sample sizes as separate lines
plot(NULL,xlim=c(0,0.2),ylim=c(0.0,1.0),pch=1,ann=F,frame.plot=F,cex=1.5)

mu_9<- link( m_samplesize_diffs , data=data.frame( changeprop_mean=unique(d$changeprop_mean),samplesize=rep(1,length(unique(d$changeprop_mean))) ) )
mu_9_mean <- apply( mu_9 , 2 , mean )
mu_9_ci <- apply( mu_9 , 2 , PI , prob=0.97 )
shade( mu_9_ci , unique(d$changeprop_mean) , col=col.alpha("blue",0.4) )

mu_12 <- link( m_samplesize_diffs , data=data.frame( changeprop_mean=unique(d$changeprop_mean),samplesize=rep(2,length(unique(d$changeprop_mean))) ) )
mu_12_mean <- apply( mu_12 , 2 , mean )
mu_12_ci <- apply( mu_12 , 2 , PI , prob=0.97 )
shade( mu_12_ci , unique(d$changeprop_mean) , col=col.alpha("orange",0.4) )

mu_15 <- link( m_samplesize_diffs , data=data.frame( changeprop_mean=unique(d$changeprop_mean),samplesize=rep(3,length(unique(d$changeprop_mean))) ) )
mu_15_mean <- apply( mu_15 , 2 , mean )
mu_15_ci <- apply( mu_15 , 2 , PI , prob=0.97 )
shade( mu_15_ci ,unique(d$changeprop_mean), col=col.alpha("skyblue",0.4) )

mu_20 <- link( m_samplesize_diffs , data=data.frame( changeprop_mean=unique(d$changeprop_mean),samplesize=rep(4,length(unique(d$changeprop_mean))) ) )
mu_20_mean <- apply( mu_20 , 2 , mean )
mu_20_ci <- apply( mu_20 , 2 , PI , prob=0.97 )
shade( mu_20_ci ,unique(d$changeprop_mean) , col=col.alpha("#213940",0.4) )

mu_60 <- link( m_samplesize_diffs , data=data.frame( changeprop_mean=unique(d$changeprop_mean),samplesize=rep(5,length(unique(d$changeprop_mean))) ) )
mu_60_mean <- apply( mu_60 , 2 , mean )
mu_60_ci <- apply( mu_60 , 2 , PI , prob=0.97 )
shade( mu_60_ci , unique(d$changeprop_mean), col=col.alpha("yellow",0.4) )

legend(x="topright",legend=c(lty2="9",lty1="12",lty3="15",lty2="20",lty3="60"),lty=c(1,1,1,1,1),lwd=c(5,5,5,5,5),col=c(col.alpha("blue",0.4),col.alpha("orange",0.4),col.alpha("skyblue",0.4),col.alpha("#213940",0.4),col.alpha("yellow",0.4)),cex=1,bty="n")
...
**Figure 7.** A difference in the proportion of time an individual spends at a location when food is present before the flexibility manipulation vs. after can be detected if the sample size is at least 20.
