...
Half of the individuals (manipulated group) are exposed to feeders that open at irregular times until they meet the passing criterion, while the other half (control group) are exposed to feeders that open regularly (Figure 3). The irregular opening times represents environmental heterogeneity, and those individuals who have enough experience in this heterogeneous environment through multiple exposures are able to learn to improve their flexibility (latency to approach the right feeder at the right time - i.e., when it is open). In both designs, the passing criterion is tailored to each population based on prior knowledge on this or similar experiments, and data are collected on success/survival measures either before and after the flexibility intervention or only after the intervention (depending on feasibility and how the study design needs to be tailored for each population). **KELSEY do we want to say anything about all of the "piloting" we're doing in GTGR in space use? Actually, we might want publish a stage 2 that combines flexmanip, space use, and flexforaging to see whether the flexibility manipulated individuals had less predictable space use and wider foraging breadth. If we can make a new RR type at PCI RR (indie RRs), then I would say we would want to register the new stage 2 after the current stage 1 is accepted. Because this stage 1 has level 6 bias (no data are collected before IPA), and the new stage 2 will use data that are already being collected. At PCI RR, the stage 1 drops to the lowest level of bias so if we included the stage 2 based on existing data we would drop to a level 1 bias.** The social learning component is conducted during this experiment.
...
60 individuals per population will be tested (50% in the control condition, 50% in the flexibility manipulation condition). Sample size rationale: the minimum sample size of 60 was determined using G*Power for a similar analysis in McCune et al. [2020](http://corinalogan.com/Preregistrations/gspaceuse.html#H2:_P2_-_Space_use_behaviors_vary_among_populations_across_the_range). **KELSEY did Cody run a minimum sample size model in space use? If so, let's use this instead. Or should we just do a GPower model with 1 predictor variable (control vs manipulated) bc this will apply generally?**  Data collection stopping rule: Data collection will be stopped when the minimum sample size is reached or when the season in which the minimum sample size is reached comes to an end.

### Randomization and counterbalancing

Individuals will be randomly assigned to the control and flexibility manipulation conditions using a random number generator (random.org).

### Protocols

 - [Great-tailed grackle protocol](https://docs.google.com/document/d/1pTrcy7ztgYVSAP7rGA6Y_lAmZ7hK_-mB01zwy0oKh7w/edit?usp=sharing) for the feeder experiment
  ...
  - Social learning data collection: are observers more likely to touch a feeder or attempt to solve it faster if they see another (demonstrator) touch the feeder compared with observers who don't see demonstrators? **- NEED TO FIGURE THIS OUT** 
...
*Response variable:* whether the individual chose the rewarded or unrewarded option per trial

### Design 2 - Feeder experiment

*Response variable:* latency for the individual to go to the open feeder
