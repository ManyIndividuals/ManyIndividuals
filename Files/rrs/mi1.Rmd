...
shade( mu_10_ci , unique(d$changeprop_mean) , col=col.alpha("blue",0.4) )
...
shade( mu_20_ci , unique(d$changeprop_mean) , col=col.alpha("orange",0.4) )
...
shade( mu_30_ci ,unique(d$changeprop_mean), col=col.alpha("skyblue",0.4) )
...
shade( mu_40_ci ,unique(d$changeprop_mean) , col=col.alpha("#213940",0.4) )
...
shade( mu_60_ci , unique(d$changeprop_mean), col=col.alpha("red",0.4) )
...
mtext("Proportion of estimates below zero",side=2,line=3, cex=1.2)
...
![If the difference in the proportion of time an individual spends at a location when food is present before the flexibility manipulation vs. after is 0.1, on average 93% of the posterior of the model based on a sample size of 20 individuals will be larger than zero, which means the model is quite certain that there is a difference that is larger than zero (12 times more likely to say that the difference is larger than zero than it is to say it is smaller than zero).](mi1FigPowerAnalysisFalseNegativeGTGR.png)
