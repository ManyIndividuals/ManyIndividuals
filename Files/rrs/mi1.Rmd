...
#write.csv(simresults,file="simresultsTouSurvival.csv") #put the outputs into a csv file so don't have to run the model again...it takes about 9 hours
```
...
# We run the STAN model - it is a binomial model that assumes that the probability to estimate a difference depends on an interaction between the sample size and the difference between the conditions. These two models do the same thing with different versions of the response variable. In m_samplesize_diffs, the response variable is already a proportion. In m_samplesize_diffscount, the response variable is a count and the model turns it into a proportion. We ended up using the latter
m_samplesize_diffs <- ulam(
    alist(
        proportion_estimates_below_zero ~ dnorm(mu,sigma),
        mu <- a[samplesize] + b[samplesize]*changeprop_mean,
...
mu_10<- link( m_samplesize_diffscount , data=data.frame( changeprop_mean=as.factor(unique(d$changeprob_mean)),samplesize=rep(1,length(unique(d$changeprob_mean))) ) )
mu_10_mean <- apply( mu_10 , 2 , mean )
mu_10_ci <- apply( mu_10 , 2 , PI , prob=0.97 )
...
mu_20 <- link( m_samplesize_diffscount , data=data.frame( changeprob_mean=as.factor(unique(d$changeprob_mean)),samplesize=rep(2,length(unique(d$changeprob_mean))) ) )
mu_20_mean <- apply( mu_20 , 2 , mean )
mu_20_ci <- apply( mu_20 , 2 , PI , prob=0.97 )
...
mu_30 <- link( m_samplesize_diffscount , data=data.frame( changeprob_mean=as.factor(unique(d$changeprob_mean)),samplesize=rep(3,length(unique(d$changeprob_mean))) ) )
mu_30_mean <- apply( mu_30 , 2 , mean )
mu_30_ci <- apply( mu_30 , 2 , PI , prob=0.97 )
...
mu_40 <- link( m_samplesize_diffscount , data=data.frame( changeprob_mean=as.factor(unique(d$changeprob_mean)),samplesize=rep(4,length(unique(d$changeprob_mean))) ) )
mu_40_mean <- apply( mu_40 , 2 , mean )
mu_40_ci <- apply( mu_40 , 2 , PI , prob=0.97 )
...
mu_60 <- link( m_samplesize_diffscount , data=data.frame( changeprob_mean=as.factor(unique(d$changeprob_mean)),samplesize=rep(5,length(unique(d$changeprob_mean))) ) )
mu_60_mean <- apply( mu_60 , 2 , mean )
mu_60_ci <- apply( mu_60 , 2 , PI , prob=0.97 )
