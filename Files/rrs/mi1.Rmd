...
Number of different habitats per individual ~ Time + (Time | ID)
...
y is the response variable: the total number of different habitats used per individual. There will be one intercept, $\alpha$, and one slope $\beta$ per individual, which will be estimated for the two times, before (and after) the manipulation. A normal distribution was used because the response variable is a sum without an expected skew to the curve [see Figure 10.6 in @mcelreath2020statistical]. The Bayesian model was developed using @mcelreath2020statistical as a guide.

###### Power analysis

We estimated our power to detect differences between conditions at different sample sizes and with different mean changes in the total number of different habitats used per individual in the before vs. after conditions (Figure 3). We simulated the number of habitats used for different sample sizes of individuals before and after the flexibility manipulation. We analyzed these simulated data with the model we will use to analyze the actual data, estimating the change in the number of habitats used between the before and after conditions. From the posterior estimates of the model, we extracted both the mean change as well as the ratio of the posterior estimates that were below zero.

If the mean ratio of estimates below zero is close to 0.5, the model assumes that the change in the number of habitats used before the flexibility manipulation is similar to after. If the ratio is close to zero, the model assumes individuals have changed their behavior. For changes smaller than 0.1, models are likely to assume that no changes occurred even with large sample sizes. If the change in the number of habitats used before the flexibility manipulation vs. after is 0.1, on average 93% of the posterior of the model based on a sample size of 20 individuals will be larger than zero. This means that the model is quite certain there is a difference that is larger than zero. In addition, none of the models for a sample size of 20 at the mean change of 0.1 have a ratio larger than 0.3, meaning that the risk of having a false negative is unlikely.

In general, with sample sizes at or above 20 and mean changes in the number of habitats used is 0.1 or larger, then it is highly likely that the model will indicate that individuals have changed their behavior. Mean changes below 0.1 can still be detected, however there is a higher risk that there will be a false negative. If the change in the number of habitats used before the flexibility manipulation vs. after is 0.025, on average 61% of the posterior of the model based on a sample size of 20 individuals will be larger than zero. In addition, only 20% of the models for a sample size of 20 at the mean change of 0.025 have a ratio larger than 0.3, meaning that the risk of having a false negative is still low.
...
changenumber <- rnorm(N_ind, mean=numberbefore/2,sd=1.5) #DIETER if had a large number before, have a smaller change relative to the after condition. Divide by 2 = assume that on average they will improve by is 2 habitats (the before mean of 5 divided by 2)
...
meanchangenumber1<-5 
meanchangenumber2<-4
meanchangenumber3<-3 #e.g., a bird spends 0.6 of its time being rewarded at cafes and dumpsters before the manipulation and then increases this proportion to 0.8 after the manipulation, showing that the manipulation improved their foraging success
meanchangenumber4<-2
meanchangenumber5<-1
meanchangenumber6<-0
...
for (samplesize in 1:length(samplesizes)) {
        ...
        indchange <- rnorm(N_ind, mean=currentchangenumber,sd=1.5) #choose the next number change for each individual
        ...
        #choose the next number change for each individual. If had a large number before, have a smaller change relative to the after condition. Multiply by 0.2 = assume that on average they will improve by 0.2. Modifying the number after relative to the number before accounts for those individuals who were already successful before the manipulation and thus won't have much of a change in number due to the manipulation. Using the relative change also ensures that an individual will never have an after number smaller than 1. Multiply currentchangenumber by 2 bc want the relative change to be in about the same order of magnitude as the proportion of change for each individual as the absolute change. If the number before is 0.5, the relative change will be the same as the absolute change. If the relative before >0.5, then the absolute change will be smaller. E.g., if number before = 0.7, then a relative change of 0.2 leads to an absolute change of 0.14. If it is <0.5, then the absolute change will be larger than 0.2.
        changenumber <- rnorm(N_ind, mean=numberbefore/2,sd=1.5) #DIETER need to check that I modified this correctly
        ...
        #sets up the data sheet with the simulated data that the model will run on
      dat <- list(
    habitatuse = rnorm(N_ind*2,mean=5,sd=2), #estimate 20 total individuals (*2 conditions) with a mean of 5 habitats/individual
...
sj <- ulam(
    alist(
        ...
        alpha ~ normal(4.5,2), #5=mean number of habitats used, 2=habitats for the sd
...
#str(post) #check it
diff_b <- post$b_ind[,2] - post$b_ind[,1] #log-odds difference p.341 McElreath 2020
#PI(diff_b) #did the difference cross zero?
#precis( list( diff_b=diff_b ) )

  # enter all of the results into the data sheet
    simresults[counter,]$n <- N_ind
    simresults[counter,]$changenumber_mean <- currentchangenumber
    simresults[counter,]$changenumber_sd <- 1.5
    simresults[counter,]$repetition <- repetition
    simresults[counter,]$number_before <- currentnumber_before
    simresults[counter,]$proportion_estimates_below_zero <- sum(diff_b<0)/length(diff_b)
    simresults[counter,]$count_estimates_below_zero <- sum(diff_b<0)
    counter<-counter+1 
...
#write.csv(simresults,file="simresultsjaytesting.csv") #put the outputs into a csv file so don't have to run the model again...it takes hours
...
#load the data sheet that was made from the simulation in the previous R chunk
d <- read.csv("https://raw.githubusercontent.com/ManyIndividuals/ManyIndividuals/main/Files/rrs/mi1simresultsjay.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
#head(d)
...
lines(x=c(-0.008,-0.008),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(0.022,0.022),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(0.044,0.044),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(0.092,0.092),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(0.192,0.192),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(0.292,0.292),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
lines(x=c(0.392,0.392),y=c(min(d[d$n %in% 10 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero),max(d[d$n %in% 10 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero)),col=col.alpha("blue",0.5))
...
lines(x=c(-0.004,-0.004),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(0.024,0.024),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(0.048,0.048),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(0.096,0.096),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(0.196,0.196),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(0.296,0.296),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
lines(x=c(0.396,0.396),y=c(min(d[d$n %in% 20 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero),max(d[d$n %in% 20 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero)),col=col.alpha("orange",0.5))
...
lines(x=c(0.028,0.028),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(0.052,0.052),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(0.1,0.1),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(0.2,0.2),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(0.3,0.3),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
lines(x=c(0.4,0.4),y=c(min(d[d$n %in% 30 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero),max(d[d$n %in% 30 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero)),col=col.alpha("skyblue",0.5))
...
lines(x=c(0.004,0.004),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(0.032,0.032),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(0.056,0.056),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(0.104,0.104),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(0.204,0.204),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(0.304,0.304),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
lines(x=c(0.404,0.404),y=c(min(d[d$n %in% 40 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero),max(d[d$n %in% 40 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero)),col=col.alpha("#213940",0.5))
...
lines(x=c(0.008,0.008),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 0,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(0.036,0.036),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 0.025,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(0.06,0.06),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 0.05,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(0.108,0.108),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 0.1,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(0.208,0.208),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 0.2,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(0.308,0.308),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 0.3,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
lines(x=c(0.408,0.408),y=c(min(d[d$n %in% 60 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero),max(d[d$n %in% 60 & d$changenumber_mean %in% 0.4,]$proportion_estimates_below_zero)),col=col.alpha("red",0.5))
...
# Axis labels (side 1=x axis, side 2=y axis)
#mtext("Sample size",side=3,line=0,at=1)
#mtext("Mean change in number of habitats used between before and after conditions",side=1,line=3, cex=1.2)
#mtext("Density of the proportion of estimates below zero",side=2,line=3, cex=1.2)
```

**Figure 4.** Curves of the mean ratio of estimates below zero (vertical lines show the minimum and maximum ratios), which illustrates the power we have to detect differences between conditions at different sample sizes. Across all models, the standard deviation of the mean change in proportion was 0.1 (a conservative estimate) and the proportion of time spent in the presence of food before the flexibility manipulation was 0.6.

```{r jayq3simulationfigurecurve, eval=F}
# (not shown in the article, but useful if you want to learn more about this modeling process)
...
dat <- list(
    habitatuse = rnorm(40,mean=4.5,sd=2), #estimate 20 total individuals (*2 conditions) with a mean of 4.5 habitats/individual (halfway between the max 10 suburban and 8 natural habitats)
    before = c(rep(1,N_ind),rep(2,N_ind)), #1 refers to before and 2 to after
    ind = rep(1:N_ind,2) #go through all individuals twice because there are observations from before and after for each
)
