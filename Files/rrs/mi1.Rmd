...
average_number_before<-c(3/10, 4.5/10, 6/10)
...
for (samplesize in 1:length(samplesizes)) {
        ...
        for (i in 1:N_ind) {
          ...
          individual_probs_after[i]<-individual_probs_before[i]+rnorm(1,mean=currentchangenumber,sd=0.1) # We increase the individual baseline values by one of the six change numbers, with variation so that some individuals might also decrease their value from their baseline before
        ...
        #sets up the data sheet with the simulated data that the model will run on
      dat <- list(
    habitatuse = c(simulatedindividuals_number_before,simulatedindividuals_repetition_numberafter), #compare before and after. standardize so can compare natural habitats with max 8 habitats with suburban with max 10 habitats
...
post <- extract.samples(sj)
  ...
  # enter all of the results into the data sheet
    ...
    simresults[counter,]$repetition <- repetition
    simresults[counter,]$changeprop_sd <- 0.1 #change this number and in individual_probs_after to test different SDs
