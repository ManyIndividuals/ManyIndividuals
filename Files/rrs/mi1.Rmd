...
#habitat: natural
datn <- list(
  proportion_estimates_below_zero_n = as.numeric(d$proportion_estimates_below_zero_habitatnatural),
  changeprop_mean_n = as.numeric(d$changeprob_mean),
  samplesize_n = as.integer(as.factor(d$n)
))

#habitat: suburban
dats <- list(
  proportion_estimates_below_zero_s = as.numeric(d$proportion_estimates_below_zero_habitatsuburban),
  changeprop_mean_s = as.numeric(d$changeprob_mean),
  samplesize_s = as.integer(as.factor(d$n)
))
...
mu_10n<- link( m_samplesize_diffsnatural , data=data.frame( changeprop_mean_n=as.numeric(unique(d$changeprob_mean)),samplesize=rep(1,length(unique(d$changeprob_mean))) ) )
...
mu_20n <- link( m_samplesize_diffsnatural , data=data.frame( changeprop_mean_n=as.numeric(unique(d$changeprob_mean)),samplesize=rep(2,length(unique(d$changeprob_mean))) ) )
...
mu_30n <- link( m_samplesize_diffsnatural , data=data.frame( changeprop_mean_n=as.numeric(unique(d$changeprob_mean)),samplesize=rep(3,length(unique(d$changeprob_mean))) ) )
...
mu_40n <- link( m_samplesize_diffsnatural , data=data.frame( changeprop_mean_n=as.numeric(unique(d$changeprob_mean)),samplesize=rep(4,length(unique(d$changeprob_mean))) ) )
...
mu_60n <- link( m_samplesize_diffsnatural , data=data.frame( changeprop_mean_n=as.numeric(unique(d$changeprob_mean)),samplesize=rep(5,length(unique(d$changeprob_mean))) ) )
...
mtext("A. Habitat: natural",side=3,line=1,at=0.15,cex=1.5)
mtext("Mean difference in survival (control vs. manipulated)",side=1,line=3, cex=1.2)
mtext("Ratio of estimates below zero",side=2,line=3, cex=1.2)
...
mu_20s <- link( m_samplesize_diffssuburban , data=data.frame( changeprop_mean=as.numeric(unique(d$changeprob_mean)),samplesize=rep(2,length(unique(d$changeprob_mean))) ) )
...
mu_30s <- link( m_samplesize_diffssuburban , data=data.frame( changeprop_mean=as.numeric(unique(d$changeprob_mean)),samplesize=rep(3,length(unique(d$changeprob_mean))) ) )
...
mu_40s <- link( m_samplesize_diffssuburban , data=data.frame( changeprop_mean=as.numeric(unique(d$changeprob_mean)),samplesize=rep(4,length(unique(d$changeprob_mean))) ) )
...
mu_60s <- link( m_samplesize_diffssuburban , data=data.frame( changeprop_mean=as.numeric(unique(d$changeprob_mean)),samplesize=rep(5,length(unique(d$changeprob_mean))) ) )
...
# Axis labels (side 1=x axis, side 2=y axis)
mtext("Sample size",side=3,line=-0.7,at=0.26)
mtext("B. Habitat: suburban",side=3,line=1,at=0.15,cex=1.5)
mtext("Mean difference in survival (control vs. manipulated)",side=1,line=3, cex=1.2)
mtext("Ratio of estimates below zero",side=2,line=3, cex=1.2)
par(op)
```
...
**Figure 18.** Curves of the mean ratio of estimates below zero (vertical lines show the minimum and maximum ratios), which illustrates the power we have to detect differences between conditions at different sample sizes. Across all models, the survival probability of the control individuals was 0.6.
...
mt <- ulam(
    alist(
        ...
        transpars> matrix[parentpairID,2]:alpha <- compose_noncentered( sigma_parentpairID , L_Rho_parentpairID , z_parentpairID ),
        matrix[2,parentpairID]:z_parentpairID ~ normal( 0 , 1 ),
        transpars> matrix[habitat,2]:beta <- compose_noncentered( sigma_habitat , L_Rho_habitat , z_habitat ),
        matrix[2,habitat]:z_habitat ~ normal( 0 , 1 ),
        # fixed priors
        g[condition] ~ normal(0,1),
        vector[2]:sigma_parentpairID ~ dexp(1),
        cholesky_factor_corr[2]:L_Rho_parentpairID ~ lkj_corr_cholesky( 2 ),
