...
###### The model (as in @logan2020xpop)

y ~ $\alpha$[habitat]

y is the response variable (phi and lambda, which are extracted from the correct and incorrect choices in the serial reversals). There is one intercept, $\alpha$, per habitat (suburban or natural) and we will estimate the habitat's average and standard deviation of the response variable. EXPLAIN PHI AND LAMBDA
...
m1 <- ulam(
    alist(
        response ~ dbinom(1,p),
        logit(p) <- g[tid] + alpha[season,tid] + h[fledglingsex] + i[caregiversex] + j*hatchdate + k[parentpairid],
        # adaptive priors - non-centered which is a parameterization that improves sampling for the MCMC model fit by helping with complex varying effects (see p.453)
        transpars> matrix[season,2]:alpha <- compose_noncentered( sigma_season , L_Rho_season , z_season ),
        matrix[2,season]:z_season ~ normal( 0 , 1 ),
        # fixed priors
        g[tid] ~ normal(0,1),
        h[fledglingsex] ~ normal(0,1),
        i[caregiversex] ~ normal(0,1),
        j ~ normal(0,1),
        k[parentpairid] ~ normal(0,1),
        vector[2]:sigma_season ~ dexp(1),
        cholesky_factor_corr[2]:L_Rho_season ~ lkj_corr_cholesky( 2 ),
        # compute ordinary correlation matrixes from Cholesky factors
        gq> matrix[2,2]:Rho_season <<- Chol_to_Corr(L_Rho_season)
    ) , data=dat , chains=4 , cores=4 , log_lik=TRUE , cmdstan = TRUE)
