...
```{r gtgr, eval=T}
# SIMULATE the population (p.437 McElreath 2021)
# estimates for pre-manipulation data: use a sequential distribution from 0.00 to 1.00 to estimate priors (n=20 data points)
d <- seq(0.00, 1.00, length.out=20) #make a sequence of 20 data points from 0.00 to 1.00
mean(d) #0.50
sd(d) #0.31

# make priors
a <- 0.50 #avg pre-manipulation proportion time spent when food present
b <- (0.20) #avg difference pre and post manipulation - unknown so estimate something higher
sigma_a <- 0.31 #std dev in intercepts (std dev a)
sigma_b <- 0.11 #std dev in slopes: std dev pre - std dev post. Post is unknown so estimate something smaller than for a like 0.20
rho <- (0.70) #correlation between intercepts and slopes (corr between pre and post). Post is unknown. Estimate a positive correlation because post should be higher than pre

# build a covariance matrix that treats standard deviations and correlations separately, then matrix multiplies them to make the covariance matrix (p.438 McElreath 2021)
Mu <- c(a, b)
sigmas <- c(sigma_a,sigma_b) # standard deviations
Rho <- matrix( c(1,rho,rho,1) , nrow=2 ) # correlation matrix
Sigma <- diag(sigmas) %*% Rho %*% diag(sigmas) # now matrix multiply to get covariance matrix

# simulate individuals, each with their own intercept and slope
N_ind <- 20 #number of individuals tested

# sample randomly using a multivariate Gaussian distribution defined by Mu and Sigma
library(MASS)
vary_effects <- mvrnorm( N_ind , Mu , Sigma )

# pull vary_effects columns into separate vectors
a_ind<- vary_effects[,1]
b_ind <- vary_effects[,2]

# visualize intercepts and slopes as well as the distribution's contours
plot( a_ind , b_ind , col=4 , xlab="intercepts (a_ind)" , ylab="slopes (b_ind)" )
# overlay population distribution
library(ellipse)
for ( l in c(0.1,0.3,0.5,0.8,0.99) )
    lines(ellipse(Sigma,centre=Mu,level=l),col="black")

# simulate observations (p.440)
N_datapoints <- 2 #simulate 2 data points per individual: one proportion for before the manipulation, and one for after
before <- rep(0:1,N_datapoints*N_ind/2)
ind_id <- rep( 1:N_ind , each=N_datapoints )
mu <- a_ind[ind_id] + b_ind[ind_id]*before
sigma <- 0.5  # std dev within cafes
wait <- rnorm( N_datapoints*N_ind , mu , sigma )
d <- data.frame( ind=ind_id , before=before , wait=wait )
